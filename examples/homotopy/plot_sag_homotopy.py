from copt.stochastic_homotopy import minimize_sag_homotopy_cgm
import json
...
# for beta_scaling in [500,1000,2000,5000]:
for beta_scaling in [3000]:
    for beta0 in [6]:
        ...
        non_negativity_constraint = NonnegativeConstraint(C_mat.shape, None, None,
                                                        name='nonnegativity', beta_scaling=beta_scaling)
        ...
        filename = 'stats_beta={beta0}_scaling={beta_scaling}.txt'.format(
            beta0=beta0,
            beta_scaling=beta_scaling)

        with open(filename, 'a', buffering=1) as statsfile:
            cb = HomotopyTrace(stats_filehandle=statsfile)

            minimize_sag_homotopy_cgm(
